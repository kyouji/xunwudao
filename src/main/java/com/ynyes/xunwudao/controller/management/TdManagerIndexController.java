...
//		} 
        ...
        //	新咨询
        List<TdApply> newApplyList = tdApplyService.findByStatusId(0L);
       
        if (null != newApplyList) {
        	 res.put("newApply", newApplyList.size());
		}else{
			res.put("newApply", 0);
		}
        
        //咨询查询
        List<TdApply> tdApplyList = tdApplyService.findAll();
        if (null != tdApplyList) {
			res.put("apply", tdApplyList.size());
		}else{
			res.put("apply", 0);
		}
