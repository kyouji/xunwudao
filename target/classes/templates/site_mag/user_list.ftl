...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="ltable">
  ...
  <tr class="odd_bg">
    ...
    <th align="center" width="10%">用户姓名</th>
    <th width="12%">手机号</th>
    <th width="8%">最近登录</th>
    <th width="12%">积分</th>
    <th width="6%">状态</th>
    ...
    <#if user_page??>
        <#list user_page.content as user>
            <tr>
                ...
                <td align="center"><#if user.totalPoints??>${user.totalPoints?c!""}</#if></td>
                <td align="center"><#if user.statusId??><#if user.statusId==0>待审核<#elseif user.statusId==1>正常</#if></#if></td>
                <td align="center">
                    <a href="/Verwalter/user/edit?id=${user.id?c}&roleId=${roleId!""}">编辑</a> | 
                    <a href="/Verwalter/user/rf/list?id=${user.id?c}">查看分销列表</a></td>
              </tr>
        </#list>
    </#if>
